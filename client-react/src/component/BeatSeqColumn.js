...
const BeatSeqColumn = (props) => {
    ...
    useEffect(() => {//runs once when just log in
        const tempbeatseq = JSON.parse(localStorage.getItem("tempbeatseq"));

        if ((beatseqId === undefined && tempbeatseq !== null) || (beatseqId === "" && tempbeatseq !== null)) {//log in
            ...
            axios
                ...
                .then((response) => {
                    ...
                    console.log("posted a new machine to MongoDB from localStorage", response);
                    //get the new object_id
                    setNewMachineId(response.data._id);
                    setMachineCreated(true);
                })
