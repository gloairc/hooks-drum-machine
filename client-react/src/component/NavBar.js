    ...
    >
      ...
      {loggedIn ? (
        <Nav className="ml-auto" id="navBar-right-account">
          <Navbar.Text><span id="welcome-name">Welcome {user.username}</span></Navbar.Text>

          <NavDropdown alignRight title={<AccountCircle />} id="basic-nav-dropdown">
            ...
            <NavDropdown.Divider />
            <NavDropdown.Item as={Link} to="/logout">Logout <span class="align-middle"><ExitToApp /></span></NavDropdown.Item>
          </NavDropdown>
        </Nav>
      ) : (
